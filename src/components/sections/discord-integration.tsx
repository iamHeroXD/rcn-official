...
import { ShieldCheck, MessageSquare, Terminal, Zap } from "lucide-react";
...
const DiscordMessage = ({ user, content, time, delay }: any) => (
  <motion.div
    ...
    className="flex gap-4 p-4 hover:bg-white/[0.02] transition-colors group rounded-lg"
  >
    ...
    <div className="flex-1">
      <div className="flex items-center gap-2 mb-1">
        <span className="text-white font-bold text-sm group-hover:underline cursor-pointer">{user}</span>
  ...
  return (
    <section className="py-24 px-4 bg-black relative">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-16 items-center">
        ...
        <div className="flex-1 w-full max-w-2xl bg-[#313338] rounded-2xl overflow-hidden shadow-2xl border border-white/5 relative">
          {/* Fake Sidebar */}
          <div className="flex h-full">
            <div className="w-16 bg-[#1E1F22] hidden md:flex flex-col items-center py-4 gap-4">
              <div className="w-10 h-10 rounded-2xl bg-prime-purple flex items-center justify-center text-white font-black text-xs">R</div>
              <div className="w-10 h-10 rounded-[20px] bg-[#313338] hover:rounded-2xl transition-all duration-300 flex items-center justify-center text-white/30">+</div>
            </div>
